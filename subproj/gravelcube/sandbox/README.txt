...
- Spectre type 2 (indirect jumps)
    ...
    maximum impact: revealing the full contents of the parent's address space, including the full
...
- Less weird, or better documented, naming policy in the kernel, these foobar/__foobar pairs confuse me (there's even a ___sys_sendmsg)
...
- O_BENEATH or similar; it keeps getting proposed, but never gets merged
    ...
    openat2 exists, but seccomp can't inspect its flags
...
cloexec is, in theory, easy; just iterate over the file descriptors, fcntl(F_GETFD), close
 everything with cloexec. In practice, which file descriptors? The only way to list open file
 descriptors on Linux is /proc/self/fd/, which isn't available. We can't ask the broker for a list
 either; while we can tell it our PID with SOCK_PASSCRED, using it would be a TOCTTOU. Another
...
For example, there is a system called Libretro <https://www.libretro.com/>; a large number of
 ...
 API that can be used by various programs ("frontends"). If the attacker can load arbitrary content
 into a core, there's a lot of exposed attack surface (and 'the core itself isn't malicious' is an
 assumption I'd rather not make), but if the cores are sandboxed, said attack surface becomes
 irrelevant.
