...
public:
	...
	//This callback will be called if the widget is altered by resize(). The rest of Arlib avoids calling callbacks for API-sourced
	// calls, but the video driver isn't the one who called resize().
	...
	//ondestroy is called whenever the viewport is destroyed, if the viewport isn't disconnected (by calling this one again) first.
