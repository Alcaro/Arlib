...
static void do_test(cstring enc, cstring dec)
{
	...
	if (enc && enc[enc.length()-1] == '=') declen_exp++;
	if (enc && enc[enc.length()-2] == '=') declen_exp++;
...
test("base64", "string,array", "base64")
{
	...
	testcall(do_test("dXJlLg==",     "ure."     ));
	testcall(do_test("cmUu",         "re."      ));
	testcall(do_test("ZS4=",         "e."       ));
	testcall(do_test("Lg==",         "."        ));
	testcall(do_test("",             ""         ));
	...
	// TODO: figure out how to implement these in this architecture, or find a better architecture
	//assert_eq(string(base64_dec("YXN1cmUu")),     "asure."   );
	//assert_eq(string(base64_dec("c3VyZS4")),      "sure."    );
	//assert_eq(string(base64_dec("dXJlLg")),       "ure."     );
	//assert_eq(string(base64_dec("cmUu")),         "re."      );
	//assert_eq(string(base64_dec("ZS4")),          "e."       );
	//assert_eq(string(base64_dec("Lg")),           "."        );
	//assert_eq(string(base64_dec("")),             ""         );
	//assert_eq(string(base64_dec("YXN1cmUu ")),     "asure."   );
	//assert_eq(string(base64_dec("c3VyZS4 ")),      "sure."    );
	//assert_eq(string(base64_dec("dXJlLg ")),       "ure."     );
	//assert_eq(string(base64_dec("cmUu ")),         "re."      );
