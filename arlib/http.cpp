#ifdef ARLIB_SOCKET
#include "http.h"
#include "file.h" // used only by get_any
...
bool http_t::can_keepalive(const req& q)
{
	return
		sock &&
		sock_keepalive_until > timestamp::now() &&
		sock_loc.same_origin(q.loc) &&
		true;
}

bool http_t::can_pipeline(const req& q)
{
	return (q.method == "GET" || q.method == "") && q.body.size() == 0 && can_keepalive(q);
}

void http_t::send_request(const req& q)
{
	cstring method = q.method;
	if (!method) method = (q.body ? "POST" : "GET");
	
	sock.send_buf(method, " ", q.loc.path, " HTTP/1.1\r\n");
	...
	if (!httpHost) sock.send_buf("Host: ", q.loc.host, "\r\n");
...
async<bytearray> http_t::get_any(cstring uri, mksocket_t mksocket)
{
...
#endif
		co_return file2::readall_array(http_t::urldecode(uri.substr(strlen(discard_prefix), ~0)));
	}
	co_return file2::readall_array(uri.c_str());
}
...
static int n_socks;
static async<autoptr<socket2>> mksock_wrap(bool ssl, cstring host, uint16_t port)
{
	n_socks++;
...
co_test("http 10", "tcp", "http")
{
	...
	http_t::req rq = { "https://floating.muncher.se/arlib/files.php" };
	// <?php
...
co_test("http 11", "tcp", "http")
{
	...
	http_t::req rq = { "https://floating.muncher.se/arlib/echo.php" }; // same as an earlier test
	rq.body = blob;
...
co_test("http_t::get_any", "runloop,http,file", "")
{
	assert_eq(cstring(co_await http_t::get_any("arlib/arlib.h")).substr(0, 12), "#pragma once");
#ifdef __unix__
	assert_eq(cstring(co_await http_t::get_any("/bin/sh")).substr(0, 4), "\x7F""ELF");
	assert_eq(cstring(co_await http_t::get_any("file:///bin%2fsh")).substr(0, 4), "\x7F""ELF");
#endif
#ifdef _WIN32
	assert_eq(cstring(co_await http_t::get_any("C:/windows/notepad.exe")).substr(0, 2), "MZ");
	assert_eq(cstring(co_await http_t::get_any("file:///C:%2Fwindows%2fnotepad.exe")).substr(0, 2), "MZ");
#endif
	assert_eq((co_await http_t::get_any("file://arlib/arlib.h")).size(), 0); // absolute paths only
	test_skip("kinda slow");
	assert_eq(cstring(co_await http_t::get_any("https://floating.muncher.se/arlib/test.txt")), "hello world");
}
#endif
#endif
