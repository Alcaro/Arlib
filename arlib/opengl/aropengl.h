...
#undef AROPENGL_GEN_HEADER
	...
	//TODO: move generated.c into arlib.h, generated.c can't change per project
	//use some funny tricks to minimize the amount of actual code in arlib.h
	//struct of function pointers that this one inherits from? CRTP?
	//use this as base class and arlib fptr struct inherits from from this one? if it's autogenerated,
	// it doesn't need to have the fptrs at the start of the object, it could easily return the offset, or a pointer to its first member
	//as for hasExtension and enableDefaultDebugger (and d3dsync), best solution would probably be having them use their own fptr structs
	//also need a fallback for non-optimized builds, probably containing the current generated.c
