...
{
	if (pipe[0] != -1) loop->set_fd(pipe[0], bind_this(&output::update), NULL);
	this->loop = loop;
}
...
void process::output::update(uintptr_t)
{
	if (pipe[0] != -1)
	{
//again: ;
		...
		if (bytes < 0 && errno == EAGAIN) goto finish;
...
finish:
	bool has_idle = (idle_id != 0);
	bool want_idle = (on_readable && buf.size() != 0);
	if (has_idle != want_idle)
	{
		if (want_idle)
		{
			idle_id = loop->set_idle(bind_this(&output::idle_cb));
		}
		else
		{
			loop->remove(idle_id);
...
bool process::output::idle_cb()
{
	update();
	if (buf.size() != 0) on_readable();
	return true;
}
