...
struct image : nocopy {
	...
	//Input color is argb8888 no matter what format the input image is. Like the other functions, overflow is ignored.
	...
	//Acts like the applicable init_decode_<fmt>. Returns false on failure; if this happens, every member should be considered invalid.
	bool init_decode(arrayview<byte> data);
	
	//Always emits valid argb8888. May (but is not required to) set the image to bargb or xrgb instead, if it's degenerate.
